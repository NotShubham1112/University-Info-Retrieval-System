...
async function ViewerStatsSection() {
...
async function ViewerStudentsSection({ q }: { q: string }) {
  ...
  if (students.length === 0) {
    return (
      <p className="text-sm text-muted-foreground">
        No students found{q ? ` for "${q}"` : ""}. Seed data may be missing — run <code>npm run db:seed</code>.
      </p>
    );
  }
  return (
        ...
        <TableBody>
          {students.map((s) => (
            <TableRow key={s.id}>
              ...
              <TableCell>
                <span className="rounded-full bg-muted px-2 py-0.5 text-xs">{s.status ?? "—"}</span>
              </TableCell>
              <TableCell className="text-right">
                <Link href={`/students/${s.id}`} className={buttonVariants({ variant: "ghost", size: "xs" })}>
                  View
                </Link>
              </TableCell>
            </TableRow>
...
export default async function ViewerPage({
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-xl font-semibold tracking-tight">Viewer — Read-only</h1>
        <p className="text-sm text-muted-foreground">
          No mutation controls. For edits, request <code>admin</code> access. Data comes from <code>student_summary</code> via cached{" "}
          <code>search_students</code> RPC (<code>search:</code> 10s TTL) and <code>dashboard_stats</code> (<code>stats:</code> 30s TTL).
        </p>
      </div>
      ...
      <Card>
        <CardHeader className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          <span className="text-xs text-muted-foreground">
            Read-only table from <code>student_summary</code> via <code>search_students</code>
          </span>
        </CardHeader>
        <CardContent className="space-y-4">
          <ViewerSearchForm q={q} />
          <Suspense fallback={<StudentsSkeleton />}>
            <ViewerStudentsSection q={q} />
          </Suspense>
          <p className="text-xs text-muted-foreground">
            Showing up to 20 results{q.trim().length > 0 ? ` filtered by "${q}"` : ""}. No edit/delete controls in viewer — switch to{" "}
