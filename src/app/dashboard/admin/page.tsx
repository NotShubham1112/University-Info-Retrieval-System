...
async function fetchUpcomingExams(): Promise<Array<{ id: number; course_id: number; semester_no: number; exam_type: string; date: string | null; status: string; course_name?: string | null }>> {
  ...
  return getCachedOrSet<Array<{ id: number; course_id: number; semester_no: number; exam_type: string; date: string | null; status: string; course_name?: string | null }>>(
    ...
    async () => {
      ...
      // Upcoming = date >= today or date is null but status draft/scheduled, ordered by date asc
...
export default async function AdminOverviewPage() {
  ...
  let upcoming: Awaited<ReturnType<typeof fetchUpcomingExams>> = [];

  try {
    [stats, upcoming] = await Promise.all([fetchDashboardStats(), fetchUpcomingExams()]);
  } catch (e) {
    error = (e as Error).message;
    try { stats = await fetchDashboardStats(); } catch {}
  }
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            ...
            <Link href="/dashboard/admin/exams" className="text-xs text-muted-foreground underline">View all</Link>
          </CardHeader>
          <CardContent>
            {upcoming.length === 0 ? (
              ...
              <div className="space-y-2">
                {upcoming.map((ex) => (
                  <div key={ex.id} className="flex items-center justify-between rounded border px-3 py-2 text-sm">
                    <div>
                      <div className="font-medium">{ex.course_name ?? `Course ${ex.course_id}`} · Sem {ex.semester_no}</div>
                      <div className="text-xs text-muted-foreground">{ex.exam_type} · {ex.date ?? "no date"} · {ex.status}</div>
                    </div>
